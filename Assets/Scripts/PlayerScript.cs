...
void Movement()
    ...
    void ShootLaser()
    {
        ...
        float posX = this.transform.position.x + (Mathf.Cos((transform.localEulerAngles.z - 0) * Mathf.Deg2Rad) * -laserDistance);
        float posY = this.transform.position.y + (Mathf.Sin((transform.localEulerAngles.z - 0) * Mathf.Deg2Rad) * -laserDistance);
